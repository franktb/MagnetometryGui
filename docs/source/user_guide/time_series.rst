...
Next, we recommend inspecting the time-series through the ``timeseries-window``, which can be opened through the dropdown menu ``view``.
The ``timeseries-window`` facilitates two key tasks: displaying the time-series and manipulating it.
...
.. note::
   We note that this approach assumes a constant ambient field for the duration of the window.
   A diurnal correction using data from the `Valencia observatory <https://data.magie.ie/>`_ might be included in the future.
...
This is due to the fact that some raw data values are approaching 0 nT, probably during recovering the magnetometer.
